...
function buildXdmUrlRecord(url, cardId, tags, snapshotTs) {
  return {
    _id: url,
    _adobepartners: {
      caasCard: {
...
// src/snaplogic/scripts/transformChimeraCardsByUrl.ts
function processResponse(response, responseIndex, transformedCards, urlToCards, logger) {
  ...
  for (var ci = 0; ci < cardCount; ci += 1) {
    ...
    if (!rawCard) {
      ...
      for (var ui = 0; ui < transformed.ctaHrefs.length; ui += 1) {
...
function logDuplicateUrls(urlToCards, logger) {
  var duplicateCount = 0;
  var urls = Object.keys(urlToCards);
  for (var i = 0; i < urls.length; i += 1) {
    var url = urls[i];
    if (urlToCards[url].length > 1) {
      duplicateCount += 1;
      logger.info("Duplicate URL: ".concat(url, " (card IDs: ").concat(urlToCards[url].join(", "), ")"));
    }
  }
  if (duplicateCount > 0) {
    logger.info("Found ".concat(duplicateCount, " duplicate URL(s)"));
  }
}
function buildXdmRecordsFromUrls(transformedCards, snapshotTs) {
  var seenUrls = [];
  var xdmRecords = [];
  for (var ti = 0; ti < transformedCards.length; ti += 1) {
    var card = transformedCards[ti];
    for (var hi = 0; hi < card.ctaHrefs.length; hi += 1) {
...
function executeScript(self) {
  ...
  self.log.info("Total cards collected: ".concat(transformedCards.length, " from ").concat(responseIndex, " response(s)"));
  logDuplicateUrls(urlToCards, self.log);
  var xdmRecords = buildXdmRecordsFromUrls(transformedCards, snapshotTs);
  self.log.info("Outputting ".concat(xdmRecords.length, " unique URL records from ").concat(transformedCards.length, " cards"));
